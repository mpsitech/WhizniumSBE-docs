...
card|engine, HMI client|each card is displayed in one browser tab - when database-backed representing a table or subset, else user-defined|
...
control complex|HMI client|specific pattern of control and sub-controls, having a specific look-and-feel in the web-based UI|
DDS|engine, M2M client|data distribution service ; optional engine thread powered by RTI(R) Connext(TM) DDS software|
dialog|engine, HMI clients|optionally wizard-style sub-elements to cards, typically to manage a complex action - as opposed to simple view and control functionality handled in panels|
...
DOM|accessor app|document object model ; storage object for session state information in analogy to a web-browser's DOM|
engine|engine|main executable responsible for performing all actions in multi-threaded fashion. Handles sessions and interactions (operation distribution/colleciton) with operation engines|
...
feature check|database, engine|"question to a record": using the WhizniumSBE expression language, the content of individual table fields can be queried (true/false answer). This is e.g. used to show/hide sub-panels on database-backed cards|
...
input file|project configuration|the import/export complexes which are used to provide WhizniumSBE with a project's model information|
...
job|engine|jobs are classes within the engine which are responsible for specfic functionality. At run-time, there is one root job object, one session job per session, HMI jobs for each card/panel/dialog/query currently active ; user-defined jobs with arbitrary functionality are also possible|
...
list functionality|database|helper table(s) or table column(s) allowing to perform conditional e.g. insert/remove/shuffle record operations on a super table (display only)|
...
machine|project configuration|computer / deployment target|
machine type|project configuration|grouping entity for machines with common e.g. library paths|
main table|database|table the entries of which make sense without other tables - and which warrant a dedicated HMI card|
...
OPC UA|engine, M2M client|OPC Unified Architecture, M2M communication protocol for industrial automation|
operation|engine, operation engine|atomic method which is executed in the operation engine by invocation from the engine|
operation engine|operation engine|satellite executable responsible for running running operations at the request of the engine|
...
operation pack|project configuration, engine, operation engine|grouping entity for operations of related functionality ; in a project, dedicated operation engine components can be defined comprising a subset of operation packs|
operation processor|operation engine|thread waiting for invocation dispatches for actions to be performed using the operation "run" methods. The number of operation processors is set in the preferences file to match the host machine's capabilities|
panel|engine, HMI client|sub-elements to cards for view and control functionality|
presetting|engine|basic type variables which can be set/reset and attached to jobs. They can be queried up the job structure, allowing to set a context for entire branches of the job tree|
...
stage|engine|state within a job's state machine - as opposed to a accessor app stage|
...
subset|database|subset to a table for records fulfilling a simple SQL WHERE condition|
...
tag|HMI client|localized control caption ; WhizniumSBE maintains a list of 100's of tags as simple as "yes"/"no" to generate the basic HMI fabric|
Title|database, HMI client|human-readable name of a record, typically not indexed - as opposed to sref's which may serve as a table record's identifier|
